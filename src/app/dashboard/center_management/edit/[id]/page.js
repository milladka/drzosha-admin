...
export default function EditCenterPage({ params }) {
    const { id } = use(params);
    ...
    useEffect(() => {
        const getData = async () => {
            if (id) {
                await AxiosInstance.get(`/admin/get_center/${id}`)
                    .then(res => {
                        if (res.data?.data) {
    ...
    const submitForm = async () => {
        ...
        if (data.image) {
            ...
            await AxiosInstance.postForm('/utility/upload_profile',
                ...
                .then(async res => {
                    if (!res.data.error) {
                        setData(prev => ({ ...prev, image_id: res.data.id }));
                    }
                })

        }

        await AxiosInstance.postForm(`/admin/edit_center/${id}`,
            {
                name: data.name,
            ...
            <div className="p-1 lg:p-4">
                ...
                >
                    <div className="grid grid-cols-2 border-b border-slate-200 p-1 lg:p-2 mb-5 lg:mb-10">
                        <div className="font-bold text-sm lg:text-lg text-slate-800 items-center flex">ویرایش مرکز درمانی</div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 lg:gap-5">
                        ...
                        <div>
                            <div>
                                ...
                                {
                                    data.image_id && <><img src={`${PICTURE_URL + data.image_id}`} alt="Doctor Profile" className="mt-2 w-24 h-24 rounded object-cover" /><button onClick={() => setData(prev => ({ ...prev, ['image_id']: '' }))} className="text-xs text-red-800">حذف</button></>
                                }
                            </div>
                        </div>
                        <div></div>
